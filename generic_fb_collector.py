...
def main(search_term, connection):
    ...
    print(search_term)
    request_count = 0
    while has_next and not already_seen and request_count < 50:
        request_count += 1
        try:
            results = None
        ...
        if total_ad_count > 0 and float(old_ad_count) / float(total_ad_count) > .98:
            already_seen = True
    ...
    ad_insert_query = "INSERT INTO ads(archive_id, creation_date, start_date, end_date, currency, page_id, snapshot_url, text, ad_sponsor_id, is_active, link_caption, link_description, link_title, country_code, funding_entity) VALUES "
    ad_count = 0
    for ad in new_ads:
        ad_insert_query += cursor.mogrify("(%s, %s, %s, %s,%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s),", (ad.archive_id, ad.creation_date, ad.start_date, ad.end_date, currency, ad.page_id, ad.image_url, ad.text, existing_ad_sponsors[ad.sponsor_label], ad.is_active, ad.ad_creative_link_caption, ad.ad_creative_link_description, ad.ad_creative_link_title, country_code, ad.sponsor_label)).decode('utf-8')
        ad_count += 1
