...
def insert_demos(cursor, new_demo_groups):
    ...
    for key, val in new_demo_groups.items():
        insert_demo_groups += cursor.mogrify("(%s, %s),",
                                             (val[0], val[1])).decode('utf-8')
        demo_group_count += 1
...
def main(search_term, connection):
    ...
    while has_next and not already_seen and request_count < 40:
        ...
        for result in results['data']:
            ...
            curr_ad = AdRecord(archive_id, page_id, image_url, ad_text,
                               ad_sponsor_label, start_date, start_date,
                               end_date, is_active, min_impressions,
                               max_impressions, min_spend, max_spend, currency,
                               link_caption, link_description, link_title)
            ...
            if is_active or archive_id in active_ads or archive_id not in ad_ids:
                ...
                for demo_result in result['demographic_distribution']:
                    ...
                    if demo_key not in existing_demo_groups:
                        new_demo_groups[demo_key] = (demo_result['gender'],
                                                     demo_result['age'])

                    new_ad_demo_impressions.add(
                        SnapshotDemoRecord(
                            archive_id, demo_result['age'],
                            demo_result['gender'],
                ...
                for region_result in result['region_distribution']:
                    ...
                    new_ad_region_impressions.add(
                        SnapshotRegionRecord(
                            archive_id, region_result['region'],
        ...
        if total_ad_count > 0 and float(old_ad_count) / float(
                total_ad_count) > .75:
            already_seen = True
    ...
    ad_count = 0
    for ad in new_ads:
        ad_insert_query += cursor.mogrify(
            "(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s),",
            (ad.archive_id, ad.creation_date, ad.start_date, ad.end_date,
             currency, ad.page_id, ad.image_url, ad.text,
             existing_ad_sponsors[ad.sponsor_label], ad.is_active,
             ad.ad_creative_link_caption, ad.ad_creative_link_description,
             ad.ad_creative_link_title, country_code)).decode('utf-8')
        ad_count += 1
    ...
    for impression in new_impressions:
        impressions_insert_query += cursor.mogrify(
            "(%s, %s, %s, %s, %s, %s),",
            (impression.archive_id, crawl_date, impression.min_impressions,
             impression.min_spend, impression.max_impressions,
             impression.max_spend)).decode('utf-8')
        impression_count += 1
    ...
    for impression in new_ad_demo_impressions:
        impression_demo_insert_query += cursor.mogrify(
            "(%s, %s, %s, %s, %s, %s, current_date),",
            (impression.archive_id,
             existing_demo_groups[impression.gender + impression.age_range],
             impression.min_impressions, impression.min_spend,
             impression.max_impressions, impression.max_spend)).decode('utf-8')
        impression_count += 1
    ...
    for impression in new_ad_region_impressions:
        impression_region_insert_query += cursor.mogrify(
            "(%s, %s, %s, %s, %s, %s, current_date),",
            (impression.archive_id, existing_regions[impression.name],
             impression.min_impressions, impression.min_spend,
             impression.max_impressions, impression.max_spend)).decode('utf-8')
        impression_count += 1
...
AdRecord = namedtuple('AdRecord', [
    'archive_id', 'page_id', 'image_url', 'text', 'sponsor_label',
    'creation_date', 'start_date', 'end_date', 'is_active', 'min_impressions',
    'max_impressions', 'min_spend', 'max_spend', 'currency',
    'ad_creative_link_caption', 'ad_creative_link_description',
    'ad_creative_link_title'
])
PageRecord = namedtuple('PageRecord', ['id', 'name'])
SnapshotRegionRecord = namedtuple('SnapshotRegionRecord', [
    'archive_id', 'name', 'min_impressions', 'max_impressions', 'min_spend',
    'max_spend', 'crawl_date'
])
SnapshotDemoRecord = namedtuple('SnapshotDemoRecord', [
    'archive_id', 'age_range', 'gender', 'min_impressions', 'max_impressions',
    'min_spend', 'max_spend', 'crawl_date'
])
...
field_list = [
    ...
    "page_id", "page_name", "region_distribution", "spend", "ad_creative_body",
    "funding_entity", "ad_creative_link_caption",
