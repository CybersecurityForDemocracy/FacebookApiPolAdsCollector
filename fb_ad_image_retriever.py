...
VIDEO_PREVIEW_IMAGE_URL_REGEX = re.compile(URL_REGEX_TEMPLATE %
                                           VIDEO_IMAGE_URL_JSON_NAME)
...
AdImageRecord = collections.namedtuple('AdImageRecord', [
    'archive_id', 'fetch_time', 'downloaded_url', 'bucket_url',
    'image_url_fetch_status', 'sim_hash', 'image_sha256'
])
...
def get_image_url_list(archive_id, snapshot_url):
    ...
    if VIDEO_IMAGE_URL_JSON_NAME in ad_snapshot_text:
        logging.debug(
            '%s found snapshot. Assuming ad has video with preview image.',
            VIDEO_IMAGE_URL_JSON_NAME)
        image_url_list = search_for_image_urls_by_regex(
            VIDEO_PREVIEW_IMAGE_URL_REGEX, ad_snapshot_text)
        if image_url_list:
            return image_url_list

        logging.info(
            '%s found in archive ID %s snapshot, but regex %s did not match.',
            VIDEO_IMAGE_URL_JSON_NAME, archive_id,
            VIDEO_PREVIEW_IMAGE_URL_REGEX)

    if IMAGE_URL_JSON_NAME in ad_snapshot_text:
        logging.debug('%s found in snapshot. Assuming ad has image.',
                      IMAGE_URL_JSON_NAME)
        image_url_list = search_for_image_urls_by_regex(IMAGE_URL_REGEX,
                                                        ad_snapshot_text)
        if image_url_list:
            return image_url_list
    ...
    logging.warning(
        'Expected JSON element not found in ad snapshot: ("%s" OR "%s")',
        IMAGE_URL_JSON_NAME, VIDEO_IMAGE_URL_JSON_NAME)
...
def search_for_image_urls_by_regex(search_regex, ad_snapshot_text):
    ...
    if not matched_strings:
        logging.debug(
            'Unable to locate image url in ad snapshot using regex: "%s"',
            search_regex)
        # TODO(macpd): raise appropriate error here.
        return None
    image_url_list = []
    for match in matched_strings:
        ...
        logging.debug('Found raw image URL value in ad snapshot: "%s"',
                      raw_image_url_str)
...
class FacebookAdImageRetriever:
    ...
    def log_stats(self):
        logging.info(
            ...
            'Images uploaded to GCS bucket: %d', self.num_snapshots_processed,
    ...
    def process_archive_images(self, archive_id_batch):
        ...
        for image_url in image_url_to_archive_id:
            ...
            bucket_url = self.store_image_in_google_bucket(
                image_dhash, image_bytes)
            ad_image_records.append(
                AdImageRecord(archive_id=image_url_to_archive_id[image_url],
                              fetch_time=fetch_time,
                              downloaded_url=image_url,
                              bucket_url=bucket_url,
                              image_url_fetch_status=int(
                                  ImageUrlFetchStatus.SUCCESS),
    ...
    try:
        with get_database_connection(config) as db_connection:
            ...
            if max_archive_ids == -1:
                ...
                archive_ids = db_interface.n_archive_ids_without_image_hash(
                    max_archive_ids)

        with get_database_connection(config) as db_connection:
            bucket_client = make_gcs_bucket_client(GCS_BUCKET,
                                                   GCS_CREDENTIALS_FILE)
            image_retriever = FacebookAdImageRetriever(db_connection,
                                                       bucket_client,
