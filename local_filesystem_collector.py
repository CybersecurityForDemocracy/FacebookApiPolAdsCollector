...
class LocalFileSystemCollector:
	...
	def __init__(self, country_code, data_path, data_field, config, insert = False, commit = False):
	...
	def selectExistingAds(self, debugEmptySets = False):
		...
		print("            Ads = {:6d} ({:d} active)".format(len(self.ad_ids), len(self.active_ads)))
		print("        Regions = {:6d}".format(len(self.existing_regions)))
		print("    Demo groups = {:6d}".format(len(self.existing_demo_groups)))
		print("          Pages = {:6d}".format(len(self.existing_pages)))
		print("    Ad sponsors = {:6d}".format(len(self.existing_ad_sponsors)))
	...
	def parseResponseBody(self, results, crawl_date):
		if "data" in results:
			...
			print("      Old ad count = {:6d}".format(self.old_ad_count))
			print("    Total ad count = {:6d}".format(self.total_ad_count))

	def previewNewNonAds(self):
		print("Prepare to insert new ad sponsors, pages, regions, and demo groups")
		print("    Ad sponsors = {:6d}".format(len(self.new_ad_sponsors)))
		print("          Pages = {:6d}".format(len(self.new_pages)))
		print("        Regions = {:6d}".format(len(self.new_regions)))
		print("    Demo groups = {:6d}".format(len(self.new_demo_groups)))
	...
	def insertNewAds(self):
		...
		print("    writing " + str(len(self.new_impressions)) + " impressions to db")
...
def main():
	...
	print("Insert = {:s} (option --insert)".format("TRUE" if insert else "false"))
	print("Commit = {:s} (option --commit)".format("TRUE" if commit else "false"))

	config = configparser.ConfigParser()
	config.read(config_filename)
	collector = LocalFileSystemCollector(country_code, data_path, data_field, config, insert = insert, commit = commit)
	collector.run()
